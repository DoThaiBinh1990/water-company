...
const SyncReviewModal = ({
  ...
  useEffect(() => {
    if (isOpen && preparedData.length > 0) {
  ...
  return (
    <Modal
      ...
      style={{
        ...
        content: { position: 'relative', margin: 'auto', width: '95%', maxWidth: '1000px', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '0', border: '1px solid #ccc', borderRadius: '8px', background: '#f9fafb' }
      }}
          ...
          <div className="space-y-4">
            {preparedData.map((p, index) => (
              <div key={p._id} className={`p-4 rounded-lg shadow-md border ${p.isDuplicateInNewSystem ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-white'}`}>
                ...
                {p.isDuplicateInNewSystem && (
                  <p className="text-sm text-red-700 mb-3 p-2 bg-red-100 rounded-md">
                    <FaExclamationTriangle className="inline mr-2" />
                    Công trình này (trùng tên, đơn vị, năm) đã tồn tại trong hệ thống mới và sẽ không được đồng bộ.
                  </p>
                )}

                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                  {/* Hiển thị các trường tự động điền */}
                    ...
                    <div className="md:col-span-2 p-3 bg-green-50 border border-green-200 rounded-md mb-2">
                      ...
                      {Object.entries(p.autoFilledFields).map(([key, value]) => (
                        <div key={key} className="grid grid-cols-1 sm:grid-cols-3 gap-1 items-center mb-0.5">
                          ...
                          {/* Hiển thị giá trị không cho sửa */}
                  ...
                  {/* Hiển thị các trường cần nhập thủ công */}
                  {p.missingMandatoryFields.map(mf => (
                    <div key={mf.field}>
                      ...
                      {/* mf.optionsSource được backend trả về */}
                  ...
                  {/* Hiển thị các trường khác trong syncData (chỉ hiển thị, không cho sửa) */}
                   {Object.entries(p.syncData)
                    .filter(([key]) => 
                        !p.missingMandatoryFields.find(mf => mf.field === key) && 
                        !p.autoFilledFields[key] &&
                        !systemLockedFields.includes(key) 
                    ).map(([key, value]) => (
                     <div key={key}>
                       <label className="block text-xs font-medium text-gray-700 mb-0.5">{getFieldLabel(key, p.projectType)}</label>
                       {/* Hiển thị giá trị dưới dạng text, không cho sửa */}
                       <div 
                         className="w-full p-1.5 text-xs border border-gray-200 bg-gray-100 rounded-md shadow-sm min-h-[28px] flex items-center"
                       >
